    ...
    <div class="layui-row">
        ...
        <div class="layui-col-xs12 layui-col-sm8 layui-col-md6">
            <form class="layui-form layui-form-pane"
                  action="${pageContext.request.contextPath}/admin/user/officeAuthorAdd/"
                  method="post">
                <div class="layui-form-item" style="text-align: left">
                    <div class="layui-form-item">
                        用户名:${user.staffname}<br>
                        <input type="hidden" name="userId" value="${user.id}"/>
                        <c:forEach items="${offices}" var="office">
                            <c:choose>
                                <c:when test="${fn:contains(officeIds, office.id)}">
                                    <input type="checkbox" name="officeIds" value="${office.id}" lay-skin="primary"
                                           title="${office.name}" checked>
                                </c:when>
                                <c:otherwise>
                                    <input type="checkbox" name="officeIds" value="${office.id}" lay-skin="primary"
                                           title="${office.name}">
...
<script>
    ...
    setTimeout(webpageClose, 100)//10s钟后关闭
    layui.use(['element', 'form', 'jquery'], function () {
        ...
        //监听CheckBox
        form.on('checkbox(officeIds)', function (data) {
