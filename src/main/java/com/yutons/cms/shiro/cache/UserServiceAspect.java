package com.yutons.cms.shiro.cache;
...
public class UserServiceAspect extends BaseCacheService{
     ...
     */
    @Pointcut("target(com.yutons.cms.service.admin.UserServiceImpl)")
    ...
    private void put(User user){
        super.put(idPrefix + user.getId(),user);
        // 建立了一个 用户名前缀和 id 之间的关系
        super.put(usernamePrefix + user.getUsername(),user.getId());
    }
}
