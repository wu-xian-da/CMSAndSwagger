...
<head>
    <meta charset="utf-8">
    <meta name="viewport"
          content="width=device-width, dept-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <title>部门列表</title>
</head>
    ...
    <div class="layui-row">
        ...
        <div class="layui-col-xs12 layui-col-sm12 layui-col-md12">
            <table class="layui-table"
                   lay-data="{height: 'full-110',even: true,url:'${pageContext.request.contextPath}/admin/dept/list',limits:[10,30,50,100], limit: 10,page:true,id:'id'}"
                   lay-filter="dept">
                <thead>
                <tr>
                    <!--<th lay-data="{checkbox:true, fixed: true}"></th>-->
                    <th lay-data="{field:'id',align:'center', width:100, sort: true}">ID</th>
                    <th lay-data="{field:'name',align:'center', width:100, sort: true}">部门名称</th>
                    <th lay-data="{field:'code', align:'center',width:100, sort: true}">部门代码</th>
                    <th lay-data="{field:'parantid', align:'center',width:200, sort: true}">父级部门</th>
...
<script>
    layui.use(['table', 'element', 'jquery'], function () {
        ...
        table.on('tool(dept)', function (obj) {
            ...
            if (obj.event === 'update') {
                window.location.href = '${pageContext.request.contextPath}/admin/dept/update/' + id;
            }
            ...
            if (obj.event === 'delete') {
                ...
                layer.confirm('您确定要删除该部门吗？', {
                    ...
                    layer.closeAll();
                    $.post("${pageContext.request.contextPath}/admin/dept/deletedeptById", {id: id}, function (data) {
                        if ("success"==data) {
                            //如果修改成功,则刷新页面
            ...
            if (obj.event === 'yes') {
                ...
                //已启用,设置为禁用
                $.post("${pageContext.request.contextPath}/admin/dept/updateState", {deptId: deptId, status: 0}, function (data) {
                    var data = data;
                    if (data == "success") {
                        //如果修改成功,则刷新页面
                        //window.location.reload(); //刷新当前页面
            ...
            if (obj.event === 'no') {
                var deptId = obj.data.deptId;
                //已禁用,设置为启用
                $.post("${pageContext.request.contextPath}/admin/dept/updateState", {deptId: deptId, status: 1}, function (data) {
                    var data = data;
                    if (data == "success") {
                        //如果修改成功,则刷新页面
                        //window.location.reload(); //刷新当前页面
